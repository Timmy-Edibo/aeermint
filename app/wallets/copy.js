...
export default function Wallets() {
  ...
  const [payoutAccountStatus, setPayoutAccountStatus] = useState(false);
  ...
  const checkPayoutAccountStatus = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `${baseUrl}/payout/verify-payout-account-status`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (storedUser?.account?.interactableType === "VENDOR") {
